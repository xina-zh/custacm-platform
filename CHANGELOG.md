...
### 2026-07-14 - 重做首页精选文章区域

- 成果：首页移除个人介绍侧栏、右侧标签云和精选区之外的普通文章列表，只展示三篇精选文章；页面使用暖米色画布，三张卡统一使用同一种低饱和浅奶油棕。卡片不使用阴影；压缩高度后只突出大标题和三行简介，日期与分类收在文字区右下角，整卡支持鼠标与键盘进入文章详情。修复 scoped CSS 丢失夜间后代选择器的问题，现有黑夜切换会同步把首页画布切为深色，并把三张卡统一切为暖炭灰。
- 视觉：首页头图底部两层波浪改为使用素材透明轮廓着色，前层以页面底色半透明覆盖、后层完全落到页面底色，消除白色波浪与灰色正文画布之间的突兀色带。
- 影响：首页精选数据固定返回三篇，按管理员置顶、管理员精选、更新时间依次排序，置顶或精选不足时自动用最新文章补足；未手填简介时自动使用正文前 280 个字符生成摘要。首卡与暖米色页面画布使用不同深度的黄棕色形成模块层级；文章详情页继续保留作者介绍和右侧目录。
- 验证：Blog 聚焦样式/布局测试、精选组件测试和生产构建通过；Java 聚焦测试与全量 Maven 测试见本次交付结果。

### 2026-07-14 - 完成双前端视觉改版实现

- 成果：冻结前端改版 v3 路线，新增两套 Vue 构建共享的 light/暖黑橙语义 token、项目设计规范、双端静态讨论原型和 CI 漂移检查；Training 完成 Action Blue/暖橙角色、现代圆角、有限玻璃、分级字号与路由动效，并把统一顶栏下的登录区精修为大号中央品牌图标、紧凑账号密码输入和圆形提交按钮，品牌图标随登录页向下滚动缩小、回到顶部恢复，日间采用自然投影而夜间保留暖色外发光；Blog 完成共享 token、Element Plus 变量、导航、文章卡片、评论、写作页和个人页视觉统一，并退出 Semantic UI、将旧字体图标统一到 Lucide。
- 影响：两端顶栏、下拉和批准的弹层使用玻璃，内容表面保持实体；Training 确认框以及 Blog 头像裁剪、文章首图裁剪、图片预览和评论表情选择器补齐焦点进入/循环、Esc 与焦点归还。Training 登录页使用自身视口容器内部滚动并隐藏滚动条，原 Blog Footer 的项目与竞赛平台链接在该容器底部复现，底部内容可达且徽章缩放跟随内部滚动；Blog 外层 iframe host 改为在 `100vh` 内计算顶栏预留，消除右侧重复蓝色滚动条。Blog 页面级滚动条统一为浅灰色圆头，搜索输入聚焦时隐藏蓝色原生框并保留中性边框提示，顶栏其它控件的 Tab 焦点不再绘制蓝色矩形框。Blog 的 Semantic UI 布局 class 改为项目自有 class，39 个旧 `<i>` 图标由全局 `AppIcon` 映射到 Lucide，旧 Ali 字体资产同时删除；Router、API、主题协议、Docker 构建上下文和部署入口不变。
- 组件库评估：不启动双端组件库迁移；Blog 保留 Element Plus 并使用 Lucide，Training 保留原生 Vue 控件，共享 token 继续承担视觉统一。ant-design-vue 不进入主线，Semantic UI 已按独立阶段退出。
- 验证：Training lint、17 个测试文件共 107 项测试、类型检查和生产构建通过，登录精修及 iframe 内页脚加入后 gzip 体积较基线增长 6.7%；Blog 37 个测试文件共 119 项测试和生产构建通过，gzip 体积为 1,121,586 bytes，较阶段 1 基线下降 10.1%。核心 light/dark、主按钮与玻璃表面文字组合通过 WCAG AA 静态计算；双端开发与代理路由返回 200，主题桥聚焦测试通过。应用内浏览器运行时连接冲突，完整 1440×900、1920×1080、移动导航和登录后页面截图仍待人工验收。
