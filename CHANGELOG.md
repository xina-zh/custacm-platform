...
### 2026-07-15 - 恢复全站日间与夜间模式

- 成果：Blog 唯一顶栏账户入口左侧恢复紧凑太阳/月亮开关，日间沿用首页与赛事荣誉的暖米色体系，Training 查询、登录和管理工作区统一使用暖米色画布、浅奶油卡片与暖灰边框，不再保留纯白底；夜间统一采用文章目录的暖黑、米白与陶土橙体系，全部界面跟随同一选择。
- 影响：首次访问固定日间，用户选择持久化到 `custacm.theme` 并跨标签页、同源 Training frame 同步，但不跟随操作系统；业务图片保持原色，文章目录在日间会转为浅色；Training 自定义 OJ 下拉完整跟随夜间色板，首页编排预览复用公开首页的双主题卡片色值，比赛管理表头不再残留浅蓝色。
- 验证：Blog 44 个测试文件共 218 项、Training 19 个测试文件共 139 项通过，Training 类型检查与 lint 通过；在 1440×900 浏览器检查首页、文章目录、赛事荣誉及 Training frame 的双主题与实时同步，生产构建和文档同步检查见本次交付记录。

### 2026-07-15 - 完善首页编排、赛事荣誉与统一浅色门户

- 成果：Blog 首页改为构建内置首图、赛事/校队标志、最多十二张可暂停拖动的滚动精选图片，以及最多三组、每组固定三篇且全首页不重复的精选文章编排；滚动精选图按视口动态生成足够副本并在原生滚动、鼠标拖动、触控板、触摸和键盘操作时双向无感换轨，不再触碰首尾。训练管理端在现有文章与首页图片页面完成候选选择、排序、裁剪、回收站和统一确认交互，旧动态横幅链路退出运行范围。
- 成果：比赛与获奖记录收敛为十个规范赛事分类和受约束奖档，公开端新增赛事筛选列表、完整详情与文章关联，选手可独立控制本人奖项的公开状态和顺序；管理员继续使用添加/删除和七天回收站，不开放任意类型组合或普通编辑入口。
- 成果：两套 Vue 3 应用固定使用共享浅色语义体系，Blog 首页、赛事页、文章阅读/发布页采用 Anthropic 米白画布与毛玻璃，文章目录保留暖黑视觉；同时完善文章目录搜索/网格、训练查询筛选、个人主页头像与文章工作区。
- 影响：新增 `V038`～`V041`，分别建立首页精选组、滚动精选图片，退役旧横幅并补充缩略图，以及规范比赛奖档与个人展示顺序；前后端接口与数据库迁移必须同步发布，不能只替换单个服务。
- 验证：已运行 `mvn clean test`、`mvn clean package -DskipTests`、`./scripts/check-test-policy.sh`，训练前端 `pnpm lint`/`pnpm test`/`pnpm typecheck`/`pnpm build`，Blog `npm ci`/`npm test`/`npm run build`，以及 `./scripts/check-doc-sync.sh origin/main WORKTREE` 和 `git diff --check`。

### 2026-07-14 - 增加公开比赛与个人获奖记录

- 成果：新增比赛公开列表与详情、年份区间和类型过滤，以及管理员创建比赛、添加参赛者和奖项、删除与七天回收站恢复能力；比赛全称作为正常记录的唯一标识，个人奖和团队奖均可绑定实际平台用户及多个相关文章。
- 成果：选手可独立绑定本人公开文章，并自主设置每项奖项是否展示在公开个人名片；本人资料返回全部获奖记录和展示状态，公开名片只返回主动开启的项目。
- 影响：新增 `V037` 比赛聚合表结构、比赛类型与省级/国家级奖项约束；蓝桥杯不支持省赛，百度之星支持省赛。删除比赛会立即从公开查询和个人名片隐藏，七天内恢复时保留参赛者、奖项、文章绑定及展示偏好。
- 验证：已运行 `mvn clean test`、MySQL 8.4 `V037` 迁移及删除/恢复场景验证、`xmllint --noout platform-blog/upstream/nblog/blog-api/src/main/resources/mapper/CompetitionMapper.xml`、`./scripts/check-doc-sync.sh origin/main WORKTREE` 和 `git diff --check`。
