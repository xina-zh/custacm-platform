...
- 两套 Router 保持独立，通过 `custacm.theme=light|dark`、localStorage 和同源 frame 消息同步主题。
- 深夜主题保持暖黑橙，不反色、不覆盖成功/警告/危险语义色；`prefers-reduced-motion` 下动画立即降级。
...
玻璃 token 提供亮/暗表面、`backdrop-filter` 与实色回退，同屏不超过三层。阶段 4 只决定侧栏或分页是否值得使用玻璃；若决定采用，在主线验收后另开可选精修 PR，不在验收阶段临时改代码。

### 3. 统一交互语义，不强求主题间相同色值

统一使用 `--color-action`、`--color-action-hover`、`--color-focus-ring` 等语义 token。日间候选为 Action Blue，深夜使用低饱和暖橙变体。颜色技术上可逆，但属于高影响视觉决策，需通过对比原型由二人确认。
...
实现状态：已完成代码、聚焦测试与生产构建；日间 Action Blue / 深夜暖橙仍作为阶段 4 前的候选配色。
...
- 并排检查主题、玻璃、圆角、动效与 iframe 同步。
- 对照页面和适用状态截图。
- 确认交互色和侧栏/分页玻璃决策；后者如采用，另开精修 PR。

完成状态：双端服务和主题链路、样式契约、对比度及构建体积自动验收已通过；侧栏与分页确认继续使用实体表面。项目所有者已确认当前日间/深夜品牌表现并接受阶段 4。应用内浏览器连接异常且本机后端/Docker 未运行，1440×900、1920×1080、移动端导航及登录后业务页面的人工截图仍作为后续回归清单保留，不虚报为已完成。
...
5. 截图按“页面 × 适用状态”记录，不做无意义的完整笛卡尔积；不适用标为 N/A。核心基线包括首页 light/dark、文章正常/失败、登录、训练正常/空/失败、一个管理员表格、一个打开的 Modal 和移动端导航。
