...
两套应用共享日间/夜间语义 token。首次访问固定为日间，两份 HTML 在 Vue 挂载前应用已保存的 `custacm.theme`；Blog 唯一顶栏提供紧凑太阳/月亮开关，选择持久化并跨标签页、同源 Training frame 同步，但不读取系统配色。日间沿用首页/赛事荣誉浅色体系，夜间沿用文章目录暖黑、米白与陶土橙体系；业务图片不统一滤色。
...
公开 Blog 首页只展示带页边距的精选区，不渲染普通文章列表、个人介绍或标签云。精选区最多三组，每组先展示管理员可编辑的标题，再以第一篇横向大卡、下方并排第二和第三篇的层级布局展示固定三篇文章；组和组内文章都按持久化顺序渲染，同一篇文章在全首页不能重复。所有首图使用 16:9 媒体框和 `contain` 完整显示，不裁剪原图；卡片展示分类、标题、三行简介、日期，以及作者头像、昵称和 username。日间使用暖米色画布与低饱和浅奶油棕卡片，夜间统一映射为文章目录的暖黑表面。文章目录同样参与全局切换，日间转为米白、夜间保持原暖黑视觉。文章详情页在固定顶栏下使用 IDE 式双区：左侧窄工具栏独立滚动并组合作者名片、目录与评论入口，作者名片右侧向登录用户提供下载操作并仅向当前作者提供编辑操作；右侧阅读画布独立滚动并依次展示大标题、日期、可选首图、首图下的小号衬线简介、正文和完整评论；移动端隐藏工具栏并恢复单列页面流。
...
Header 渐隐区下方通过公开 `GET /homepage-featured-images` 一次读取最多十二张有序 1200×800 精选图片；空列表时不渲染图片带。服务端同时返回 720×480 `thumbnailUrl` 和原图 `imageUrl`，旧记录首次读取时补生成缩略图。Blog 根据视口与单组图片宽度动态生成足够多的奇数副本，从中间副本开始缓慢向左滚动；滚动进入外围区时按完整序列宽度换回中间区，像素内容与滚动相位保持一致，因此向左或向右都没有真实首尾。图片带初始只加载缩略图，点击打开共享预览器后仍先显示缩略图，用户明确点击“加载原图”才请求高清文件。悬停、键盘聚焦和拖动会暂停自动滚动，原生横向滚动、鼠标拖动、触控板、触摸与方向键仍可双向手动控制；左右覆盖层以画布渐变和 `backdrop-filter` 模糊边界。`prefers-reduced-motion: reduce` 下关闭自动滚动。
...
默认首页图片随 Blog 前端构建发布。运行时上传目录同时挂载给 Blog API（读写）和 Nginx（只读）；Nginx 直接提供 `/api/image/**` 并对 UUID 文件设置 immutable 缓存，Referer 白名单由 `FRONTEND_IMAGE_REFERER_HOSTS` 和 `FRONTEND_ALLOW_LOCAL_REFERERS` 在容器启动时生成，其他 `/api/**` 仍代理 Blog API。
