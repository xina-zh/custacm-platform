...
| --- | --- |
| `platform-blog/upstream/nblog/blog-api/` | 唯一 Spring Boot 后端；Blog、文章/头像托管图片高清与缩略图及零垃圾回收、首页图片、认证、用户资料/个人友链、用户/OJ handle、`top.naccl` 训练 HTTP adapter 与统一 Flyway runtime |
| `platform-blog/upstream/nblog/blog-view/` | Vue 3 + Vite 公开 Blog；生产路径 `/`，“我的主页”集中资料与本人文章，文章详情左侧显示当前文章作者公开名片，文章编辑支持 Markdown 导入、首图裁剪、正文图片选择/拖拽/粘贴，阅读页按需加载高清图 |
