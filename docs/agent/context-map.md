...
| `platform-blog/upstream/nblog/blog-view/` | Vue 3 + Vite 公开 Blog；生产路径 `/`，已接入共享视觉 token、Element Plus 语义变量、有限玻璃和弹层焦点管理；保留首页、文章、登录用户文章图片归档下载、本人回收站/恢复、分类、标签、搜索、支持本地 Google Noto Emoji/Unicode 的评论和 `/profile` 个人主页，桌面左右侧栏在顶栏下吸附，并持有 `/training/**` 外壳 |
