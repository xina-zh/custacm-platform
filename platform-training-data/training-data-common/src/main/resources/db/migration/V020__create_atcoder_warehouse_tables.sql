create table if not exists dwd_atcoder__submission (
    ...
    index idx_dwd_atcoder_submission_ods (ods_submission_id),
    index idx_dwd_atcoder_handle_time (handle, submitted_at_utc_plus8),
    index idx_dwd_atcoder_problem_time (problem_key, submitted_at_utc_plus8, submission_id),
    index idx_dwd_atcoder_handle_problem (handle, problem_key),
    index idx_dwd_atcoder_batch (ods_batch_id)
);

create table if not exists dwm_atcoder__handle_problem_first_accepted (
    id bigint primary key auto_increment,
    handle varchar(128) not null,
    ...
    index idx_dwm_atcoder_problem (problem_key),
    index idx_dwm_atcoder_handle_date (handle, first_accepted_date_utc_plus8)
);

create table if not exists dws_atcoder__handle_daily_rating_accepted_summary (
    id bigint primary key auto_increment,
    handle varchar(128) not null,
    ...
    index idx_dws_atcoder_date_difficulty (
        accepted_date_utc_plus8,
        difficulty
    )
);
