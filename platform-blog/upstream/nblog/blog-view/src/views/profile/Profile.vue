<template>
	<section class="profile-page ui segment">
		<template v-if="authUser">
			...
			<p v-if="ojHandlesError" class="oj-load-error" role="alert">
				{{ ojHandlesError }}
				<button type="button" @click="loadOjHandles">重试</button>
			</p>
	...
	let nextLinkKey = 1
		...
		methods: {
			...
			async loadOjHandles() {
				const token = readToken()
				if (!token) return
				this.ojHandlesLoading = true
				this.ojHandlesError = ''
				try { this.ojHandles = await getCurrentOjHandles(token) }
				catch (error) { this.ojHandlesError = this.errorMessage(error, 'OJ 账号读取失败，请重试。') }
			...
			ojHandleText(ojName) { return this.ojHandlesLoading ? '读取中…' : this.ojHandlesError ? '读取失败' : (this.ojHandles[ojName] || '未绑定') },
			ojProfileUrl(ojName) {
...
<style scoped>
	...
	.oj-load-error { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin: 10px 0 0; border-left: 3px solid #a14b4b; background: #f8eeee; color: #7b3434; padding: 9px 12px; font-size: 12px; }
	.oj-load-error button { border: 0; background: transparent; color: #17324d; font: inherit; font-weight: 700; cursor: pointer; }
