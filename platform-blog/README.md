...
两份静态产物由 `frontend/Dockerfile` 一次构建并复制进同一个 Nginx 镜像。浏览器 API 统一走 `/api/**`；Nginx 去掉 `/api` 后转发给 Blog API。两份 Vue Router 彼此独立，用户会话共享 `custacm.accessToken` 和 `custacm.user`。两端共享 `custacm.theme` 日间/夜间模式：首次固定日间，Blog 顶栏切换后持久化、跨标签页及同源 Training frame 同步，不跟随系统主题；夜间采用文章目录的暖黑、米白与陶土橙体系。
...
- 首页大图下方的精选图片带独立于精选文章：管理员最多上传十二张 1200×800 JPEG，公开 `GET /homepage-featured-images` 一次返回完整有序列表；Blog 端按视口动态复制足够多的完整序列，以可暂停、可拖动且左右渐隐模糊的无缝横向循环展示，向左或向右使用鼠标、触控板、触摸与键盘滚动都不会到达首尾。
