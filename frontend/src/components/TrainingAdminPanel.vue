<template>
  <section class="admin-workspace" aria-label="管理员操作内容">
    <nav class="training-admin-tabs" aria-label="管理员页面"><button :class="{ 'is-active': section === 'create' }" type="button" @click="emit('sectionChange', 'create')">创建用户</button><button :class="{ 'is-active': section === 'users' }" type="button" @click="emit('sectionChange', 'users')">管理用户</button><button :class="{ 'is-active': section === 'articles' }" type="button" @click="emit('sectionChange', 'articles')">管理文章</button><button :class="{ 'is-active': section === 'categories' }" type="button" @click="emit('sectionChange', 'categories')">分类与标签</button><button :class="{ 'is-active': section === 'training' }" type="button" @click="emit('sectionChange', 'training')">数据采集</button><button :class="{ 'is-active': section === 'appearance' }" type="button" @click="emit('sectionChange', 'appearance')">首页图片</button></nav>
...
import AdminUserManagementPanel from './AdminUserManagementPanel.vue'; import ArticleAdminPanel from './ArticleAdminPanel.vue'; import CategoryAdminPanel from './CategoryAdminPanel.vue'; import CreateUsersPanel from './CreateUsersPanel.vue'; import HomepageBannerAdminPanel from './HomepageBannerAdminPanel.vue'; import TrainingDataOpsPanel from './TrainingDataOpsPanel.vue';
